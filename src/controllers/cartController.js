...
//create cart for a user if it does not exist 

const createCart = async function (req, res) {
    try {
        const userId = req.params.userId
        const data = req.body
        const { productId, cartId } = data    //get cartId and productId in request body
        ...
        if (!cartData) {
            ...
            const itemArray = []
            itemArray.push({ productId: productId, quantity: 1 }) //cart item can't be 0
        ...
        if (productIdList.find(id => id.toString() == productId)) {
            const update = await cartModel.findOneAndUpdate(
                ...
                { $inc: { totalPrice: +checkProductId.price, "items.$.quantity": +1 } }, //$-Projects the first element in an array that matches the query condition.
                { new: true }
